...
(function patchPipeline(pipeline) {
	...
	(function patchPipelineDocumentSources(documentSources) {
		...
		documentSources.SortDocumentSource.prototype.getMarkdown = function(level) {
			var indent = getIndentStr(level),
				str = indent + (this.limitSrc ? "sort docs by (keeping only " + this.limitSrc.getLimit() + "):\n" : "sort docs by:\n");
	...
	(function patchPipelineExpressions(expressions) {
		...
		expressions.ToLowerExpression.prototype.getMarkdown = function(level) {
			return getIndentStr(level) + "the lower case version of string from " +
				this.operands[0].getMarkdown(level).trimLeft().trimRight() + "\n";
		};

		expressions.ToUpperExpression.prototype.getMarkdown = function(level) {
			return getIndentStr(level) + "the upper case version of string from " +
				this.operands[0].getMarkdown(level).trimLeft().trimRight() + "\n";
		};

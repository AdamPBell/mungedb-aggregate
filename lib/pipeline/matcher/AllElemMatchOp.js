...
var AllElemMatchOp = module.exports = function AllElemMatchOp(){
	...
	this._list = [];
}, klass = AllElemMatchOp, base = require("./MatchExpression"), proto = klass.prototype = Object.create(base.prototype, {constructor:{value:klass}}); //jshint ignore:line
...
proto.debugString = function debugString(level) {
	var debug = this._debugAddSpace(level) +
		this.path() + " AllElemMatchOp:" +
		(this.getTag() ? " " + this.getTag().debugString() : "") +
		"\n";
	for (var i = 0; i < this._list.length; i++) {
		debug += this._list[i].debugString(level + 1);
	}
	return debug;
};

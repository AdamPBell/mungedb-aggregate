...
var ObjectCtx = Expression.ObjectCtx = (function() {
	 ...
	 */
	var klass = function ObjectCtx(opts) {
		if (Value.getType(opts) !== "Object") throw new Error("opts is required and must be an Object containing named args");
 ...
 */
klass.parseObject = function parseObject(obj, ctx, vps) { //jshint maxcomplexity:21
	...
	for (var fieldCount = 0, n = fieldNames.length; fieldCount < n; ++fieldCount) {
		...
		if (fieldName[0] === "$") {
			...
			expression = Expression.parseExpression(fieldName, obj[fieldName], vps); //NOTE: DEVIATION FROM MONGO: see #parseExpression api
		} else {
			if (kind === OPERATOR)
				throw new Error("this object is already an operator expression, and can't be used as a document expression (at '" +
					fieldName + "'.; uassert code 15990");
				...
				case "number":
					...
					if (fieldValue) {
						...
						if (!(ctx.isTopLevel && fieldName === Document.ID_PROPERTY_NAME))
							throw new Error("The top-level " + Document.ID_PROPERTY_NAME +
								" field is the only field currently supported for exclusion; uassert code 16406");
						expressionObject.excludeId = true;
					}
					break;
				default:
					throw new Error("disallowed field type " + Value.getType(fieldValue) +
						" in object expression (at '" + fieldName + "') uassert code 15992");
			}
		}
	}
...
klass.removeFieldPrefix = function removeFieldPrefix(prefixedField) {
	if (prefixedField.indexOf("\0") !== -1)
		throw new Error("field path must not contain embedded null characters; uassert code 16419");
	if (prefixedField[0] !== "$")
		throw new Error("field path references must be prefixed with a '$' ('" + prefixedField + "'); uassert code 15982");
...
//NOTE: these must be located after klass statics have been setup
var ObjectExpression = require("./ObjectExpression"), //jshint ignore:line
	FieldPathExpression = require("./FieldPathExpression"), //jshint ignore:line
	ConstantExpression = require("./ConstantExpression"); //jshint ignore:line
